---
title: "Space Tools"
description: "Learn how to enable or disable tools for each space to keep your team focused."
---

import { Aside } from '@astrojs/starlight/components';
import ImageEnhancer from '@/components/ImageEnhancer.astro';

<ImageEnhancer />

Space admins can enable or disable the following tools for each space: Documents & Files, Discussions, Tasks, Templates, and — when it is available for your company — [KPIs](/help/intro-to-kpis). This helps keep each space focused on the workflows that matter and hides sections your team does not use.

## How to configure Space Tools

1. Navigate to the space you want to configure.
2. Click on the **"..."** icon in the top right corner.
3. Select **Configure tools** from the dropdown menu.
4. Toggle the switches to enable or disable specific tools for that space.

![Screenshot: Space tools menu](./space-tools.png)

<Aside>Disabling a tool hides it from the sidebar and navigation, but it does not delete any data associated with that tool. You can re-enable it at any time to restore access.</Aside>

## Available Tools

- **Documents & Files**: A place to share rich text documents, images, videos, and other files.
- **Discussions**: Post announcements, pitch ideas, and discuss ideas with your team.
- **Tasks**: Work together on tasks that don’t belong to a specific project.
- **Templates**: Save reusable project structures and use them for recurring work. See [Project templates](/help/project-templates).
- **KPIs**: Track the numbers this space cares about and log updates on a weekly or monthly cadence. See [Introduction to KPIs](/help/intro-to-kpis). If **KPIs** is not listed on this page, the tool is not available for your company yet.

By customizing the tools available in each space, you can create a cleaner, more relevant environment for your team.
