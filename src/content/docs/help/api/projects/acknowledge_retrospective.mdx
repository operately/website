---
title: "Projects API: Acknowledge retrospective"
description: "Acknowledge retrospective with the Operately Projects API."
---

import CurlExampleBlock from "@components/CurlExampleBlock.jsx"

## Endpoint

| Property | Value |
| --- | --- |
| Type | `mutation` |
| Method | `POST` |
| Path | `/api/external/v1/projects/acknowledge_retrospective` |
| Handler | `OperatelyWeb.Api.Projects.AcknowledgeRetrospective` |

## Authentication

- Requires a valid API token.
- Requires a write-enabled token.
- Read-only tokens return `403`.


## Inputs

<div style={{ overflowX: "auto" }}>
  <table style={{ minWidth: "56rem" }}>
    <thead>
      <tr>
        <th style={{ whiteSpace: "nowrap" }}>Field</th>
        <th style={{ whiteSpace: "normal" }}>Type</th>
        <th style={{ whiteSpace: "nowrap" }}>Required</th>
        <th style={{ whiteSpace: "nowrap" }}>Nullable</th>
        <th style={{ whiteSpace: "nowrap" }}>Default</th>
      </tr>
    </thead>
    <tbody>
      <tr>
  <td style={{ whiteSpace: "nowrap" }}><code>project_id</code></td>
  <td style={{ whiteSpace: "normal" }}><code>id</code> primitive (encoded as <code>string</code>)</td>
  <td style={{ whiteSpace: "nowrap" }}>Yes</td>
  <td style={{ whiteSpace: "nowrap" }}>No</td>
  <td style={{ whiteSpace: "nowrap" }}>-</td>
</tr>

    </tbody>
  </table>
</div>


## Outputs

| Field | Type | Required | Nullable |
| --- | --- | --- | --- |
| `retrospective` | `project_retrospective` object | Yes | No |


## cURL Example

<CurlExampleBlock client:load command={"curl --request POST \\\n  --url \"https://app.operately.com/api/external/v1/projects/acknowledge_retrospective\" \\\n  --header \"Authorization: Bearer ${OPERATELY_API_TOKEN}\" \\\n  --header \"Content-Type: application/json\" \\\n  --data '{\"project_id\":\"value\"}'"} />

## Response Example

```json
{
  "retrospective": {
    "__typename": "value",
    "acknowledged_at": "2026-01-01T09:30:00Z",
    "acknowledged_by": "<person>",
    "author": "<person>",
    "champion": "<person>",
    "closed_at": "2026-01-01",
    "content": "value",
    "id": "value",
    "notifications": [
      "<notification>"
    ],
    "permissions": "<project_permissions>",
    "potential_subscribers": [
      "<subscriber>"
    ],
    "project": "<project>",
    "reactions": [
      "<reaction>"
    ],
    "reviewer": "<person>",
    "subscription_list": "<subscription_list>"
  }
}
```

